...
CAN_frame TESLA_221 = {
    .FD = false, .ext_ID = false, .DLC = 8, .ID = 0x221, .data = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};
...
void update_CAN_frame(TESLA_221_Struct msg) {
  TESLA_221.data[0] = (msg.VCFRONT_LVPowerStateIndex << 0) | (msg.vehiclePowerState << 5);
  TESLA_221.data[1] = (msg.parkLVState << 0) | (msg.espLVState << 2) | (msg.radcLVState << 4) | (msg.hvacCompLVState << 6);
  TESLA_221.data[2] = (msg.ptcLVRequest << 0) | (msg.sccmLVRequest << 2) | (msg.tpmsLVRequest << 4) | (msg.rcmLVRequest << 6);
  TESLA_221.data[3] = (msg.iBoosterLVState << 0) | (msg.tunerLVRequest << 2) | (msg.amplifierLVRequest << 4) | (msg.das1HighCurrentLVState << 6);
  TESLA_221.data[4] = (msg.das2HighCurrentLVState << 0) | (msg.diLVRequest << 2) | (msg.disLVState << 4) | (msg.oilPumpFrontLVState << 6);
  TESLA_221.data[5] = (msg.oilPumpRearLVRequest << 0) | (msg.ocsLVRequest << 2) | (msg.vcleftHiCurrentLVState << 4) | (msg.vcrightHiCurrentLVState << 6);
  TESLA_221.data[6] = (msg.uiHiCurrentLVState << 0) | (msg.uiAudioLVState << 2) | (msg.cpLVRequest << 4) | (msg.epasLVState << 6);
  TESLA_221.data[7] = (msg.hvcLVRequest << 0) | (msg.tasLVState << 2) | (msg.pcsLVState << 4);
}
...
int main() {
  ...
  while (true) {
    ...
    if (mux0 && !printed_mux0) {
      ...
      for (int i = 0; i < 8; ++i) {
        Serial.print(TESLA_221.data[i], HEX);
        Serial.print(" ");
      }
      Serial.println();
      printed_mux0 = true;
  ...
  CAN_frame TESLA_2D1 = {
      .FD = false, .ext_ID = false, .DLC = 2, .ID = 0x2D1, .data = {0x00, 0x00}};  // 7F 01 is a common message
  ...
  void update_CAN_frame(TESLA_2D1_Struct msg) {
  ...
  CAN_frame TESLA_3A1 = {
      .FD = false, .ext_ID = false, .DLC = 8, .ID = 0x3A1, .data = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};
  ...
  uint8_t calculateCounter(TESLA_3A1_Struct & msg) {
    static uint8_t counter = 0;
    return counter++;
  }

  void update_CAN_frame(TESLA_3A1_Struct msg) {
    TESLA_3A1.data[0] = (msg.bmsHvChargeEnable << 0) |       // Bit 0, Length 1
                        (msg.preconditionRequest << 1) |     // Bit 1, Length 1
                        (msg.APGlassHeaterState << 2);       // Bit 2, Length 3
    TESLA_3A1.data[1] = (msg.LVLoadRequest << 1) |           // Bit 9, Length 1
                        (msg.diPowerOnState << 2);           // Bit 10, Length 3
    TESLA_3A1.data[2] = (msg.driverIsLeavingAnySpeed << 5);  // Bit 13, Length 1
    TESLA_3A1.data[3] = (msg._12vStatusForDrive << 6) |      // Bit 14, Length 2
    ...
    TESLA_3A1.data[4] = (msg.driverUnbuckled << 0) |         // Bit 32, Length 2
                        (msg.passengerUnbuckled << 2) |      // Bit 34, Length 2
                        (msg._2RowLeftUnbuckled << 4) |      // Bit 36, Length 2
                        (msg._2RowCenterUnbuckled << 6);     // Bit 38, Length 2
    TESLA_3A1.data[5] =
  ...
  enum VCFRONT_12vStatusForDrive { NOT_READY_FOR_DRIVE_12V = 0, READY_FOR_DRIVE_12V = 1, EXIT_DRIVE_REQUESTED_12V = 2 };

  enum VCFRONT_2RowCenterUnbuckled { NONE = 0, OCCUPIED_AND_UNBUCKLED = 1, SNA = 2 };

  enum VCFRONT_2RowLeftUnbuckled { NONE = 0, OCCUPIED_AND_UNBUCKLED = 1, SNA = 2 };

  enum VCFRONT_2RowRightUnbuckled { NONE = 0, OCCUPIED_AND_UNBUCKLED = 1, SNA = 2 };

  enum VCFRONT_APGlassHeaterState { SNA = 0, ON = 1, OFF = 2, OFF_UNAVAILABLE = 3, FAULT = 4 };
  ...
  enum VCFRONT_driverBuckleStatus { UNBUCKLED = 0, BUCKLED = 1 };

  enum VCFRONT_driverDoorStatus { OPEN = 0, CLOSED = 1 };
  ...
  CAN_frame TESLA_1F9 = {
      .FD = false, .ext_ID = false, .DLC = 1, .ID = 0x1F9, .data = {0x00}};  // 100ms ID505 VCSEC_requests
