...
struct TESLA_221_Struct {
  ...
  uint8_t vcrightHiCurrentLVState;
  uint8_t uiHiCurrentLVState;
  uint8_t uiAudioLVState;
  uint8_t cpLVRequest;
  uint8_t epasLVState;
  uint8_t hvcLVRequest;
  uint8_t tasLVState;
  uint8_t pcsLVState;
  uint8_t VCFRONT_LVPowerStateCounter;   // Counter
...
void update_CAN_frame(CAN_frame& frame, const TESLA_3A1_Struct& msg) {
  ...
  frame.data.u8[6] =
      (((uint16_t)(msg.pcsEFuseVoltage * 10) & 0x03FF) << 6) | ((uint16_t)(msg.pcs12vVoltageTarget * 100) & 0xFF);
...
void update_CAN_frame(CAN_frame& frame, const TESLA_333_Struct& msg) {
  TESLA_333.data.u8[0] = (msg.UI_openChargePortDoorRequest << 0) |   // Bit 0, Length 1
                         ...
                         0x80;                                       // Set bit 7 to 1
  TESLA_333.data.u8[1] =
      (msg.UI_acChargeCurrentLimit << 1) | 0x10;  // Bit 8, Length 7 (shift left by 1) and set bit 4 to 1
  uint16_t chargeTerminationPct = msg.UI_chargeTerminationPct * 10;  // Apply factor
  TESLA_333.data.u8[2] = (chargeTerminationPct & 0xFF);       // Bit 16, Length 10 (split across data[2] and data[3])
  TESLA_333.data.u8[3] = (chargeTerminationPct >> 8) & 0x07;  // Bit 16, Length 10 (split across data[2] and data[3])
...
void update_values_vehicle() {
  ...
  if (!printed) {
    Serial.print("Updated CAN frame data: ");
    for (int i = 0; i < 1; ++i) {
      Serial.print(TESLA_1F9.data.u8[i], HEX);
      Serial.print(" ");
    }
    Serial.println();
    printed = true;
  ...
  CAN_frame TESLA_339 = {
      .FD = false, .ext_ID = false, .DLC = 8, .ID = 0x339, .data = {0x5C, 0x41, 0x02, 0x00, 0x00, 0x03, 0x40, 0x03}};
  ...
  void update_CAN_frame(CAN_frame & TESLA_339, const TESLA_339_Struct& msg) {
    ...
    TESLA_339.data.u8[1] = (msg.VCSEC_prsntRsnHighThresholdR << 4) |     // Bit 4, Length 1
    ...
    TESLA_339.data.u8[2] = (msg.VCSEC_vehicleLockStatus << 0) |          // Bit 12, Length 4
                           (msg.VCSEC_lockRequestType << 4);             // Bit 24, Length 5
    TESLA_339.data.u8[3] = (msg.VCSEC_authenticationStatus << 0) |       // Bit 16, Length 2
    ...
    TESLA_339.data.u8[4] = (msg.VCSEC_frunkRequest << 0) |               // Bit 34, Length 2
                           (msg.VCSEC_MCUCommandType << 4);              // Bit 36, Length 3
    TESLA_339.data.u8[5] = (msg.VCSEC_lockIndicationRequest << 0) |      // Bit 51, Length 3
                           (msg.VCSEC_immobilizerState << 3);            // Bit 48, Length 3
    TESLA_339.data.u8[6] = (msg.VCSEC_rightFrontLockStatus << 0) |       // Bit 21, Length 1
                           (msg.VCSEC_rightRearLockStatus << 1) |        // Bit 22, Length 1
                           (msg.VCSEC_simpleLockStatus << 4);            // Bit 54, Length 2
    TESLA_339.data.u8[7] = (msg.VCSEC_summonRequest << 1) |              // Bit 29, Length 3
  ...
  CAN_frame TESLA_321 = {
      .FD = false, .ext_ID = false, .DLC = 8, .ID = 0x321, .data = {0xE6, 0x95, 0xA7, 0x70, 0x02, 0x6A, 0xF0, 0x12}};
  ...
  void update_CAN_frame(CAN_frame & frame, TESLA_321_Struct & msg) {
    ...
    uint8_t tempAmbientFiltered = (msg.VCFRONT_tempAmbientFiltered + 40) * 2;

    TESLA_321.data.u8[0] = (tempCoolantBatInlet & 0xFF);          // Bit 0, Length 10
    TESLA_321.data.u8[1] = ((tempCoolantBatInlet >> 8) & 0x03) |  // Bit 10, Length 2
                           (tempCoolantPTInlet << 2);
    TESLA_321.data.u8[2] = ((tempCoolantPTInlet >> 6) & 0xFF);
    TESLA_321.data.u8[3] = (tempAmbient << 0) |                       // Bit 24, Length 8
                           (msg.VCFRONT_coolantLevel << 5) |          // Bit 21, Length 1
                           (msg.VCFRONT_brakeFluidLevel << 6);        // Bit 22, Length 2
    TESLA_321.data.u8[4] = (tempAmbientFiltered << 0);                // Bit 40, Length 8
    TESLA_321.data.u8[5] = (msg.VCFRONT_battSensorIrrational << 0) |  // Bit 48, Length 1
  ...
  void update_values_vehicle() {
    ...
    if (!printed) {
      Serial.print("Updated CAN frame data: ");
      for (int i = 0; i < 8; ++i) {
        Serial.print(TESLA_321.data.u8[i], HEX);
        Serial.print(" ");
      }
      Serial.println();
      printed = true;
    ...
    static bool battery_OverDchgCurrentFault =
        false;  //Warns if the pack discharge is above max discharge current limit
    ...
    static bool battery_Supply12vFault =
      ...
      switch (rx_frame.ID) {
        case 0x352:                              // 850 BMS_energyStatus newer BMS
          ...
          if (mux == 1) {
            ...
            battery_energy_buffer_m1 = ((rx_frame.data.u8[3] << 8) |
                                        rx_frame.data.u8[2]);  //16|16@1+ (0.01,0) [0|0] "kWh"//to datalayer_extended
          ...
          battery_ideal_energy_remaining =  //BMS_idealEnergyRemaining : 33|11@1+ (0.1,0) [0|204.6] "KWh" //((_d[5] & (0x0FU)) << 7) | ((_d[4] >> 1) & (0x7FU));
              (((rx_frame.data.u8[5] & 0x0F) << 7) |
               ((rx_frame.data.u8[4] & 0x7F) >> 1));  //Example 311 * 0.1 = 31.1kWh
          battery_energy_to_charge_complete =  // BMS_energyToChargeComplete : 44|11@1+ (0.1,0) [0|204.6] "KWh"// ((_d[6] & (0x7FU)) << 4) | ((_d[5] >> 4) & (0x0FU));
              (((rx_frame.data.u8[6] & 0x7F) << 4) |
               ((rx_frame.data.u8[5] & 0x0F) << 4));  //Example 147 * 0.1 = 14.7kWh
        ...
        case 0x20A:  //522 HVP_contactorState:
          ...
          battery_fcCtrsResetRequestRequired =
              ((rx_frame.data.u8[3] >> 2) & (0x01U));                                //26|1@1+ (1,0) [0|1] ""  Receiver
        ...
        case 0x212:  //530 BMS_status: 8
          ...
          battery_BMS_chgPowerAvailable = ((rx_frame.data.u8[6] & (0x01U)) << 10) |
        ...
        case 0x224:                                                           //548 PCS_dcdcStatus:
          ...
          battery_PCS_dcdcHvBusDischargeStatus =
              ((rx_frame.data.u8[0] >> 4) & (0x03U));  //0 "IDLE" 1 "ACTIVE" 2 "FAULTED"
          ...
          battery_PCS_dcdcMaxOutputCurrentAllowed =
              ((rx_frame.data.u8[5] & (0x01U)) << 11) | ((rx_frame.data.u8[4] & (0xFFU)) << 3) |
        ...
        case 0x252:  //Limit //594 BMS_powerAvailable:
          ...
          BMS_hvacPowerBudget =
              (((rx_frame.data.u8[7] << 6) | ((rx_frame.data.u8[6] & 0xFC) >>
                                              2)));  //50|10@1+ (0.02,0) [0|20.46] "kW"  //Example 1000 * 0.02 = 20kW?
          BMS_notEnoughPowerForHeatPump = ((rx_frame.data.u8[5] >> 2) &
                                           (0x01U));  //BMS_notEnoughPowerForHeatPump : 42|1@1+ (1,0) [0|1] ""  Receiver
          BMS_powerLimitState =
              (rx_frame.data.u8[6] &
        ...
        case 0x132:  //battery amps/volts //HVBattAmpVolt
          ...
          battery_charge_time_remaining =
              (((rx_frame.data.u8[7] & 0x0F) << 8) |
               rx_frame.data
                   .u8[6]);  //ChargeHoursRemaining : 48|12@1+ (1,0) [0|4095] "Min"  //Example 228 * 0.1 = 22.8min
        ...
        case 0x3D2:  //TotalChargeDischarge:
          ...
          battery_total_charge = ((rx_frame.data.u8[7] << 24) | (rx_frame.data.u8[6] << 16) |
        ...
        case 0x292:  //BMS_socStatus
          datalayer.battery.status.CAN_battery_still_alive =
              CAN_STILL_ALIVE;  //We are getting CAN messages from the BMS
          battery_beginning_of_life =
              (((rx_frame.data.u8[6] & 0x03) << 8) | rx_frame.data.u8[5]) * 0.1;  //40|10@1+ (0.1,0) [0|102.3] "kWh"
          battery_soc_min =
              (((rx_frame.data.u8[1] & 0x03) << 8) | rx_frame.data.u8[0]);  //0|10@1+ (0.1,0) [0|102.3] "%"
          battery_soc_ui = (((rx_frame.data.u8[2] & 0x0F) << 6) |
                            ((rx_frame.data.u8[1] & 0xFC) >> 2));  //10|10@1+ (0.1,0) [0|102.3] "%"
          battery_soc_max = (((rx_frame.data.u8[3] & 0x3F) << 4) |
                             ((rx_frame.data.u8[2] & 0xF0) >> 4));  //20|10@1+ (0.1,0) [0|102.3] "%"
          battery_soc_ave =
        ...
        case 0x320:  //800 BMS_alertMatrix                                                //BMS_alertMatrix 800 BMS_alertMatrix: 8 VEH
          ...
          if (mux == 0) {                                                                 //mux0
            ...
            battery_BMS_a025_SW_PwrBalance_Anomaly =
                ((rx_frame.data.u8[3] >> 4) & (0x01U));                                      //28|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a026_SW_HFCurrent_Anomaly = ((rx_frame.data.u8[3] >> 5) & (0x01U));  //29|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a034_SW_Passive_Isolation =
                ((rx_frame.data.u8[4] >> 5) & (0x01U));                                 //37|1@1+ (1,0) [0|0] ""  X ?
            ...
            battery_BMS_a039_SW_DC_Link_Over_Voltage =
                ((rx_frame.data.u8[5] >> 2) & (0x01U));                                     //42|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a047_SW_Task_Stack_Overflow =
                ((rx_frame.data.u8[6] >> 2) & (0x01U));  //50|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a048_SW_Log_Upload_Request =
                ((rx_frame.data.u8[6] >> 3) & (0x01U));                                      //51|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a059_SW_Pack_Voltage_Sensing =
                ((rx_frame.data.u8[7] >> 6) & (0x01U));  //62|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a060_SW_Leakage_Test_Failure =
                ((rx_frame.data.u8[7] >> 7) & (0x01U));  //63|1@1+ (1,0) [0|0] ""  X
          }
          if (mux == 1) {                                                                     //mux1
            ...
            battery_BMS_a071_SW_SM_TransCon_Not_Met =
                ((rx_frame.data.u8[1] >> 6) & (0x01U));  //14|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a075_SW_Chg_Disable_Failure =
                ((rx_frame.data.u8[2] >> 2) & (0x01U));  //18|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a076_SW_Dch_While_Charging =
                ((rx_frame.data.u8[2] >> 3) & (0x01U));  //19|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a077_SW_Charger_Regulation =
                ((rx_frame.data.u8[2] >> 4) & (0x01U));                                      //20|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a084_SW_Sleep_Wake_Aborted =
                ((rx_frame.data.u8[3] >> 3) & (0x01U));                                      //27|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a087_SW_Feim_Test_Blocked = ((rx_frame.data.u8[3] >> 6) & (0x01U));  //30|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a088_SW_VcFront_MIA_InDrive =
                ((rx_frame.data.u8[3] >> 7) & (0x01U));                                   //31|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a092_SW_ChargePort_Mia_On_Hv =
                ((rx_frame.data.u8[4] >> 3) & (0x01U));  //35|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a094_SW_Drive_Inverter_MIA =
                ((rx_frame.data.u8[4] >> 5) & (0x01U));                                      //37|1@1+ (1,0) [0|0] ""  X
          ...
          if (mux == 2) {  //mux2
            battery_BMS_a121_SW_NVRAM_Config_Error =
                ((rx_frame.data.u8[0] >> 4) & (0x01U));  // 4|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a122_SW_BMS_Therm_Irrational =
                ((rx_frame.data.u8[0] >> 5) & (0x01U));                                       //5|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a132_HW_BMB_OTP_Uncorrctbl =
                ((rx_frame.data.u8[1] >> 7) & (0x01U));                                      //15|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a139_SW_DC_Link_V_Irrational =
                ((rx_frame.data.u8[2] >> 6) & (0x01U));                                     //22|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a146_SW_Brick_Overdischarged =
                ((rx_frame.data.u8[3] >> 5) & (0x01U));                                  //29|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a149_SW_Missing_Config_Block = (rx_frame.data.u8[4] & (0x01U));  //32|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a151_SW_external_isolation =
                ((rx_frame.data.u8[4] >> 2) & (0x01U));                                    //34|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a162_SW_No_Power_For_Support =
                ((rx_frame.data.u8[5] >> 5) & (0x01U));  //45|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a163_SW_Contactor_Mismatch =
                ((rx_frame.data.u8[5] >> 6) & (0x01U));  //46|1@1+ (1,0) [0|0] ""  X
            battery_BMS_a164_SW_Uncontrolled_Regen =
                ((rx_frame.data.u8[5] >> 7) & (0x01U));                                    //47|1@1+ (1,0) [0|0] ""  X
            ...
            battery_BMS_a171_SW_Stack_Voltage_Sense =
                ((rx_frame.data.u8[6] >> 6) & (0x01U));                                      //54|1@1+ (1,0) [0|0] ""  X
    ...
    CAN_frame can_msg_1CF[] = {
        ...
        {.FD = false,
         ...
         .DLC = 8,
         .ID = 0x1CF,
         .data = {0x01, 0x00, 0x00, 0x1A, 0x1C, 0x02, 0x40, 0x49}}};
    ...
    CAN_frame can_msg_118[] = {
        ...
        {.FD = false,
         ...
         .DLC = 8,
         .ID = 0x118,
         .data = {0x70, 0x8F, 0x30, 0x10, 0x00, 0x08, 0x00, 0x80}}};
...
the first, for a few cycles, then stop all  messages which causes the contactor to open. */
    ...
    void printFaultCodesIfActive() {
      ...
      printDebugIfActive(battery_OverCurrentFault,
      ...
      printDebugIfActive(battery_VerSupplyFault,
